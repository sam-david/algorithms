	...
	for (var i=0;i<digits.length;i++) {
		...
		if (firstline == true) {
			...
			for (var j=0;j<digits[i].length;j++) {
				...
				if (digits[i][index] + digits[i+1][index] + remainder >= 10) {
				...
				console.log("first + second =", digitsArray, "remainder",remainder)
			}
			firstline = false; 
		} else {
			console.log(i+1,"line subtracting");
			var tempArray = [];
			for (var j=0;j<digits[i].length;j++) {
				var index = digits[i].length - j - 1;
				if (digits[i][index] + digitsArray[index] + remainder >= 10) {
					tempArray.push(digits[i][index] + digitsArray[index] + remainder - 10);
					remainder = 1;
				} else {
					tempArray.push(digits[i][index] + digitsArray[index] + remainder);
					remainder = 0;
				}
			}
			digitsArray = tempArray;
			console.log(i+1,"line +",i+2, "line =",digitsArray);
		}
	}
	console.log("Final Digits Array", digitsArray);
}
